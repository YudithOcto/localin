            ...
            Container(
              ...
              child: RaisedButton(
                ...
                child: provider.loading
                    ...
                    : Text(
                        isUpdatePage ? 'Simpan' : 'Buat Komunitas',
                        style: TextStyle(fontSize: 18.0, color: Colors.white),
                      ),
                elevation: 5.0,
                onPressed: () async {
                  if (provider.validateInput()) {
                    var result = await provider.createCommunity(isUpdatePage);
                    ...
                    if (dialog == 'close' && result?.error == null) {
                      Navigator.of(context).pop();
                    }
                  } else {
                    if (provider.logoImageFile == null) {
                      showDialogValidate(context, 'Logo image is required');
                    } else if (provider.coverImageFile == null) {
                      showDialogValidate(context, 'cover image is required');
                    } else if (provider.logoImageFile.lengthSync() > 900000) {
                      showDialogValidate(
