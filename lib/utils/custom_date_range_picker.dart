...
class _DatePickerHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget renderDateChoosen(startDate, endDate) {
      return IgnorePointer(
        ...
        child: Padding(
          ...
          child: Semantics(
            child: Text(
              '${localizations.formatMediumDate(startDate)} ${endDate != null ? '- ${localizations.formatMediumDate(endDate)}' : '- Check-out Date'}',
              style: TextStyle(
                  fontSize: 14.0, fontWeight: FontWeight.w600, color: dayColor),
            ),
          ),
        ),
      );
    ...
    final Widget startHeader = Container(
        child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      mainAxisAlignment: MainAxisAlignment.center,
      children: <Widget>[
        renderYearButton(selectedFirstDate),
        renderDateChoosen(selectedFirstDate, selectedLastDate)
      ],
    ));
...
class DayPicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; true; i += 1) {
      ...
      // weekday starts from 1 (monday) and sunday is 7
...
class _DatePickerDialogState extends State<CustomDatePicker> {
  @override
  void initState() {
    super.initState();
...
Future<List<DateTime>> showDatePicker({
  ...
  Widget child = new CustomDatePicker(
